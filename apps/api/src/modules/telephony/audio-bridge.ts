...
export function connectToElevenLabs(session: BridgeSession): void {
  ...
  elWs.on("message", (data: Buffer | string) => {
      ...
      case "media":
        if (session.elevenLabsWs?.readyState === WebSocket.OPEN) {
          ...
          session.elevenLabsWs.send(
            JSON.stringify({ type: "audio", audio: pcmPayload }),
          );
        }
        break;
      case "stop":
...
export function cleanupSession(callId: string): void {
  ...
  const conferenceActive =
    session.warmTransfer &&
    ["caller_in_conference", "builder_ringing", "builder_joined"].includes(session.warmTransfer.status);
